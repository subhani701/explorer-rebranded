...
//
// Hardening overlay: tolerate both token-address API shapes. The frontend reads
// `token.address_hash`, but some backend versions return the field as `address`.
// When `address_hash` is missing the token link became bare `/token` (404) and
// the copy button was empty. Fall back to `address` so the link is always built.
...
const Link = chakra((props: LinkProps) => {
  const defaultHref = route(
    { pathname: '/token/[hash]', query: { ...props.query, hash: getTokenAddress(props.token) } },
...
const Copy = (props: CopyProps) => {
  return (
    <EntityBase.Copy
      { ...props }
      text={ getTokenAddress(props.token) ?? '' }
    />
  );
};
