...
//
// Rebrand overlay: detect the VoltusWave Wallet by its injected flag.
// The extension sets `window.ethereum.isVoltusWave = true` (and isMetaMask=true
// for compatibility). We match on isVoltusWave so it's identified as VoltusWave
// rather than MetaMask. The extension's own code is unchanged.
...
const isWalletProvider = (wallet: WalletType) => (provider: WalletProvider): boolean | undefined => {
  switch (wallet) {
    case 'voltuswave': {
      return Boolean((provider as WalletProvider & { isVoltusWave?: boolean }).isVoltusWave);
    }
    case 'rabby': {
