...
// Hardening overlay: guard against an undefined/empty `hash`. Hooks stay
// unconditional (React rules); hash usage is guarded so a missing hash renders
// empty instead of crashing the page (Tokens/Search loading rows).
...
const HashStringShortenDynamic = ({ hash, fontWeight = '400', noTooltip, tailLength = TAIL_LENGTH, as = 'span', tooltipInteractive, ...props }: Props) => {
  ...
  const isTruncated = Boolean(hash) && hash.length !== displayedString.length;
